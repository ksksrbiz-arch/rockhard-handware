...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  const variants = productId ? await safeQuery<any>(() => sql`
  ...
  const ratingRow = productId ? await safeOne<{ avg: string; count: number }>(() => sql`
    select coalesce(avg(rating), 0)::text as avg, count(*)::int as count
    from reviews
    where product_id = ${productId} and status = 'published'
  `) : null;

  const avgRating = ratingRow && Number(ratingRow.avg) > 0 ? Number(ratingRow.avg).toFixed(1) : null;
    ...
    <>
      ...
      <main className="flex-1">
        ...
        <div className="mx-auto max-w-4xl px-4 pb-16">
          {productId && <Reviews productId={productId} productSlug={product.slug} />}
        </div>
      </main>
      <Footer />
    </>
