...
export default async function AdminProducts() {
  const products = await safeQuery<any>(() => sql`
    select id, slug, name, price, category, status, inventory
        ...
        <table className="w-full text-sm min-w-[700px]">
          <thead className="bg-bg-alt">
            <tr className="text-left text-muted text-xs tracking-wider-2 uppercase">
              ...
              <th className="p-3">Price</th><th className="p-3">Inventory</th>
          ...
          <tbody>
            ...
            {products.length === 0 && (
              <tr><td colSpan={6} className="p-8 text-center text-muted">No products. Click &quot;+ NEW PRODUCT&quot; to add one.</td></tr>
            )}
          </tbody>
        </table>
      </div>
