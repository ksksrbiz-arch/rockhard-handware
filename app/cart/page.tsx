          ...
          {items.length === 0 ? (
            ...
            <>
              <div className="space-y-4 mb-8">
                {items.map((it) => (
                  <div key={it.slug} className="flex gap-4 border border-border bg-card p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        <button onClick={() => updateQty(it.slug, Math.max(1, it.qty - 1))} className="px-2 py-1 border border-border">−</button>
                        <span className="px-3">{it.qty}</span>
                        <button onClick={() => updateQty(it.slug, it.qty + 1)} className="px-2 py-1 border border-border">+</button>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="font-display tracking-wider-2">${(it.price * it.qty).toFixed(2)}</p>
                      <button onClick={() => removeItem(it.slug)} className="text-muted text-xs hover:text-accent mt-2">REMOVE</button>
                    </div>
                  </div>
                ))}
              </div>
